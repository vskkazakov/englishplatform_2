...
def share_category(request, student_id):
    """
    Отправка категории ученику
    """
    if request.method != 'POST':
        # Для GET-запросов тоже нужно возвращать форму
        form = CategorySharingForm(teacher=request.user)
        return render(request, 'students/share_category_modal.html', {'form': form})
    # Проверяем, что пользователь - учитель
    try:
        profile = UserProfile.objects.get(user=request.user)
        if profile.role != 'teacher':
    ...
    if form.is_valid():
        # Проверяем, что запрос на эту категорию еще не отправлен
        ...
        if existing_request and existing_request.status in ['rejected', 'cancelled']:
            existing_request.status = 'pending'
            existing_request.message = form.cleaned_data['message']
            existing_request.save()
            return JsonResponse({
                'success': True,
